...
export const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  ...
  return (
    <div className="bg-[#FAF8F4] border border-[#EFE7D8] rounded-2xl p-3 sm:p-4 flex flex-col sm:flex-row items-stretch gap-3 sm:gap-4 shadow-xs hover:shadow-md transition-all duration-300 group">
      {/* Left: Product Image */}
      <div className="w-full sm:w-2/5 shrink-0 relative aspect-square rounded-xl overflow-hidden bg-white border border-slate-100 cursor-pointer">
        <Link href={productUrl} className="block relative w-full h-full">
          ...
          />
          {product.badge && (
            <span className="absolute top-2 left-2 px-2 py-0.5 bg-red-600 text-white text-[10px] sm:text-[11px] font-bold rounded-md shadow-xs uppercase tracking-wide z-10">
              {product.badge}
            </span>
          )}
        ...
        <div className="space-y-1.5 pt-2 border-t border-amber-200/50">
          <div className="flex items-center gap-1.5">
            <button
              ...
              className="flex-1 py-2 bg-primary hover:bg-emerald-700 active:scale-[0.98] text-white font-extrabold rounded-xl transition-all text-xs shadow-sm flex items-center justify-center gap-1"
            >
            ...
            <button
              ...
              className="p-2 bg-emerald-100 hover:bg-emerald-200 text-emerald-800 rounded-xl transition-all active:scale-95 flex items-center justify-center shrink-0"
            >
            ...
            <Link
              href={productUrl}
              className="p-2 bg-white hover:bg-slate-100 border border-slate-200 text-slate-700 rounded-xl transition-all flex items-center justify-center shrink-0"
